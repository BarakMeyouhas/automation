...
const DiscordNode = ({ id, data, selected }: NodeProps<DiscordNodeType>) => {
  ...
  return (
    <div className={`relative min-w-[280px] rounded-3xl border bg-white p-4 shadow-lg transition ${selected ? 'border-[#5865F2] shadow-indigo-100' : 'border-indigo-200 shadow-slate-200/70'}`}>
      {selected ? (
        <button
          type="button"
